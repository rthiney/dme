...
const store = {
  smartSkin: config.orangeSkin, // localStorage.getItem('sm-skin') || config.orangeSkin, // make it orangeSkin by default config.smartSkin,
  skin: config.skins.find((_skin) => {
    // tslint:disable-next-line:max-line-length
    return _skin.name === config.orangeSkin; // (localStorage.getItem('sm-skin') || config.orangeSkin /* make it orangeSkin by default config.smartSkin*/);
  }),
  skins: config.skins,
...
export class LayoutService {
  ...
  onFixedPageFooter() {
    this.store.fixedPageFooter = true; // Make it always fixed. !this.store.fixedPageFooter;
    this.dumpStorage();
    this.trigger();
  }
